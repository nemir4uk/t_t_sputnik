...
from src.file_checker.file_check_utils import extract_file_metadata
...
async def main():
    ...
    async def publish_to_alert(body: bytes):
        await channel.default_exchange.publish(
            Message(
                body=body,
                delivery_mode=DeliveryMode.PERSISTENT,
    ...
    async def callback_with_retry(message: aio_pika.IncomingMessage):
        async with message.process(ignore_processed=True):
            logger.info("Check_metadata message received")
            ...
            try:
                ...
                file_item, ttl = await asyncio.to_thread(extract_file_metadata, data, retry_count)
                if file_item.processing_status == "processed" or file_item.processing_status == "failed":
                    logger.info(f"Check_metadata Success process extract_file_metadata with status {file_item.processing_status}")
                    data.scan_status = ScanStatus.success_check
                    await asyncio.gather(
                        publish_status(data.model_dump_json().encode()),
                        message.ack(),
                        publish_to_alert(data.model_dump_json().encode())
                    )
                else:
                    if retry_count < max_retries:
                        logger.error("Check_metadata Failed process extract_file_metadata -> to retry queue")
                        await publish_retry(data.model_dump_json().encode(), retry_count + 1, ttl)
                    else:
                        logger.error("Check_metadata Failed process extract_file_metadata max_retries exceeded -> to dlx queue")
                        data.scan_status = ScanStatus.failed_check
                        await asyncio.gather(
                            publish_status(data.model_dump_json().encode()),
                            publish_failed(data.model_dump_json().encode(), retry_count,
                                           "Check_metadata Failed process extract_file_metadata max_retries exceeded"),
                            message.ack()
                        )
                ...
                logger.critical(f"Check_metadata Unexpected error {e}")
                data.scan_status = ScanStatus.failed_check
                await asyncio.gather(
                    publish_status(data.model_dump_json().encode()),
                    publish_failed(data.model_dump_json().encode(), retry_count, f"Check_metadata Unexpected error {e}"),
                    message.ack()
                )
...
# main
        ...
        logger.info("Check_metadata started")
